/*
 * vc_g2d.c - VideoCore (V3D 7.1) back end of the raspi5 (Raspberry Pi 5 /
 * BCM2712) g2d layer: accelerated offline ARGB8888 drawing, ported from
 * the bare-metal g2d library (proven on real Pi 5 hardware).
 *
 * Extracted from bsp_g2d.c, which keeps only the public bsp_g2d API, the
 * argument validation and the scalar CPU paths.  This file owns everything
 * that talks to the GPU: the affine map construction shared with the
 * kernels, the eligibility gate on caller-supplied physical addresses, the
 * large-surface batching policy and every QPU dispatch.  v3d_g2d.c holds
 * the hardware bring-up, cache and submission mechanics;
 * g2d_qpu_kernels.h holds the generated instruction streams.
 *
 ...
 *
 *   gpu_fill_surface     any clipped rect        -> argb_fill kernel
 *   gpu_blit_op          any clipped dst rect    -> argb_blit kernel
 *   gpu_scale_op         whole dst               -> power-of-two scale kernel
 *                                                  for aligned exact 2^n sizes,
 *                                                  otherwise argb_blit
 *   gpu_rotate_op        right-angle content box -> argb_blit; otherwise
 *                        any angle/size         -> argb_rotate kernel
 *                       (dst < box: clipped to dst top-left; destinations
 *                      past G2D_BIG_SURFACE go out in 2D tiles - see
 *                      LARGE-SURFACE BATCHING)
 *
 * gpu_alpha_op uses a dedicated source-over kernel.  Its two-read
 ...
 *     addresses.
 *
 * LARGE-SURFACE BATCHING (gpu_copy_op / gpu_blit_op / gpu_alpha_op /
 * gpu_scale_op): a forward-walk dispatch stalls ~3-4x once its read+write
 * working set outgrows the V3D L2 (measured on the Pi 5: blit_opaque@1280x960
 * exec 11.9 ms vs rotate_180's 3.6 ms for the same byte count; the cliff
 ...
 *
 * gpu_rotate_op TILES instead of banding past the cliff: a rotated
 * walk crosses the source diagonally, so only a 2D destination tile
 * keeps both the read swath and the tile's writes cache-sized
 * (rotate_45_bbox@1920x1080 measured 58.5 ms whole-surface).  The maps
 * are evaluated at absolute destination pixels and both affine kernels
 * gate writes per lane on the rect, so a tile decomposition is exact
 * for any angle (argb_rotate's rect mode also fills out-of-content
 * samples with transparent 0).  180 degrees is exempt: its backward
 ...
 * eligible, or the dispatch failed) returns 0.  A failed dispatch is
...
/* g2d_map_t (the Q15 coefficient set identical to the kernels' uniform
 * layout) and the rotation codes are declared in <videocore/vc_g2d.h>. */
...
/* vec4 fill of a clipped dst rect (argb_fill4 kernel): each lane stores
 * 16 bytes per TMU request - 4x the bytes of argb_fill's request-rate
 * bound loop.  Caller guarantees the 16-byte contract (see the gate in
 * gpu_fill_surface). */
static int gpu_fill4_surface(uint32_t phys, uint32_t *argb,
                             int32_t w, int32_t h,
                             ...
                             uint32_t color)
{
    ...
    rpq = (H + nq - 1) / nq;
    u[0] = color;
    u[1] = phys + (uint32_t)((int64_t)y0 * w + x0) * 4u;
    u[2] = (uint32_t)w * 4u;
    u[3] = (uint32_t)C;
    u[4] = (uint32_t)(V * 16);
    u[5] = (uint32_t)H;
 ...
 * moves 16 bytes per vec4 TMU request.  (sx0, sy0) is the SOURCE pixel
 * the dst rect origin (x0, y0) maps to.  Caller guarantees the contract
 * checked by gpu_copy_eligible(). */
                            ...
                            unsigned maint)
{
    ...
    if (nq > H)
        nq = H;
    rpq = (H + nq - 1) / nq;
    u[0] = dst_phys + (uint32_t)((int64_t)y0 * dst_w + x0) * 4u;
    u[1] = src_phys + (uint32_t)((int64_t)sy0 * src_w + sx0) * 4u;
    u[2] = (uint32_t)dst_w * 4u;
    u[3] = (uint32_t)src_w * 4u;
    u[4] = (uint32_t)C;
    u[5] = (uint32_t)(V * 16);
    u[6] = (uint32_t)H;
    u[7] = (uint32_t)rpq;
 ...
 * the source (the copy kernel has no edge clamp), every walk 16-byte
 * aligned (vec4 accesses WRAP at 16-byte boundaries), the tail chunk's
 * <= 240-byte over-read still inside RAM, and a probed vec4-capable TMU.
 * The first decision is logged: a rejected gate looks identical to an
 * accepted one in the benchmark output otherwise. */
                      ...
                      int32_t x0, int32_t w, int32_t h)
{
    ...
    if ((w & 3) || (x0 & 3) || (sx0 & 3))
        why = "rect not 4px aligned";
    else if ((src_w & 3) || (dst_w & 3))
        why = "row stride not 16B aligned";
    else if ((src_phys & 15u) || (dst_phys & 15u))
        why = "surface base not 16B aligned";
    else if (sx0 < 0 || sy0 < 0 || sx0 + w > src_w || sy0 + h > src_h)
        why = "src rect out of bounds";
    else if (!v3d_g2d_phy_valid((ewokos_addr_t)src_phys,
                                (size_t)src_w * src_h * 4u + 256u))
        why = "src over-read not RAM";
    ok = (why == NULL) && v3d_g2d_vec4_ok();
    if (!logged) {
        logged = 1;
        slog("g2d copy gate: %s (src=%08x dst=%08x sw=%d dw=%d x0=%d w=%d)\n",
             ok ? "ok" : (why ? why : "vec4 unsupported"),
             src_phys, dst_phys, (int)src_w, (int)dst_w, (int)x0, (int)w);
    }
    return ok;
}
...
int gpu_fill_surface(uint32_t phys, uint32_t *argb,
                     int32_t w, int32_t h,
                     int32_t x0, int32_t y0, int32_t x1, int32_t y1,
                     uint32_t color)
{
    ...
    if (w <= 0 || h <= 0 || x0 < 0 || y0 < 0 ||
        ...
        return 0;
    /* vec4 fast path: 16-byte-aligned walks only (surface stride, rect
     * origin and width in whole 4-pixel units) and a probed TMU.  The
     * first decision is logged - see gpu_copy_eligible. */
    {
        static int logged = 0;
        int align = ((w & 3) == 0 && (x0 & 3) == 0 &&
                     ((x1 - x0) & 3) == 0 && (phys & 15u) == 0);
        int vec4 = align ? v3d_g2d_vec4_ok() : 0;
        if (!logged) {
            logged = 1;
            slog("g2d fill4 gate: %s (w=%d x0=%d rw=%d phys=%08x)\n",
                 vec4 ? "ok" : (align ? "vec4 unsupported" : "not aligned"),
                 (int)w, (int)x0, (int)(x1 - x0), phys);
        }
        if (vec4)
            return gpu_fill4_surface(phys, argb, w, h, x0, y0, x1, y1, color);
    }
...
/* Destination tile side for large-surface rotation: a 256x256 tile's
 * source swath is at most ~(256*sqrt(2))^2 pixels (~0.5 MB at 45
 * degrees, the worst angle), so the tile's reads and writes both stay
 * cache-sized.  Multiple of 16 so only the last tile column/row is
 * ragged (the kernels' per-lane rect gate covers those). */
#define G2D_ROT_TILE 256

/* Rotate in 2D destination tiles (see the header LARGE-SURFACE BATCHING
                            ...
                            int32_t dst_w, int32_t dst_h, int no_clamp)
{
    ...
    for (y0 = 0; y0 < dst_h; y0 = y1) {
        ...
        for (x0 = 0; x0 < dst_w; x0 = x1) {
            ...
            x1 = (dst_w - x0 < G2D_ROT_TILE) ? dst_w : x0 + G2D_ROT_TILE;
            if (x0 == 0 && y0 == 0)
                maint |= V3D_G2D_MAINT_PRE;
            if (x1 >= dst_w && y1 >= dst_h)
                maint |= V3D_G2D_MAINT_POST;
...
/* ------------------------------------------------------------------ */
/* whole-operation dispatch: batching policy and kernel choice          */
/* ------------------------------------------------------------------ */

/* Identity 1:1 rect copy (argb_copy kernel) over the whole rect
 * [rx,rx1) x [ry,ry1), banded past the large-surface cliff. */
 ...
 * [ry,ry1) (argb_blit kernel), banded past the large-surface cliff: the
 * ROT_0 map is row independent, so bands never touch each other's rows. */
 ...
 * [rx,rx1) x [ry,ry1) (argb_alpha kernel), banded past the large-surface
 * cliff: the blend only reads/writes the band's own dst rows, so
 * sequential bands compose correctly. */
                 ...
                 size_t dst_bytes)
{
    ...
    if (dst_bytes <= G2D_BIG_SURFACE)
        return gpu_alpha_surface(m, alpha, src_phys, argb_src, src_w, src_h,
                                 dst_phys, argb_dst, dst_w, dst_h,
                 ...
                 int32_t dst_w, int32_t dst_h)
{
    ...
    if ((dst_w & 15) == 0 && ratio_x != 0 && ratio_x == ratio_y &&
        src_w > dst_w) {
        /* downscale only: the scale_pow2 walk (group step + row extra)
         * beats the general blit loop on the Pi 5 (~800 vs ~940
         * cyc/group).  The up walk is NOT used: at 2x up its 16 lanes read
         * only 8 distinct source words (two lanes per address, plus the
         * rewind), and that duplicate-address pattern measures ~3.6x
         * slower than the blit loop on real V3D.  Upscales keep the blit
         * kernel (Q15 map, clamp-free loop - the pow2 up map stays
         * in-bounds), the same path the fractional upscale benches hit. */
    ...
    if ((size_t)dst_w * dst_h * 4 <= G2D_BIG_SURFACE)
        return gpu_affine_surface(kcode, knwords, m,
                                  ...
                                  0, 0, dst_w, dst_h, 1,
                                  V3D_G2D_MAINT_ALL);
    /* destination image memory past the cliff: scale in horizontal bands.
     * The scale_pow2 walk is only proven for whole-surface dispatches
     * (u16..u19 incremental constants), so the banded path falls back to
     * the rect-safe argb_blit kernel. */
...
/* Whole-surface rotation through m into the bw x bh content box of the
 * dst_w x dst_h destination, tiled past the large-surface cliff. */
int gpu_rotate_op(const g2d_map_t *m, int32_t rot, int32_t bw, int32_t bh,
                  ...
                  int32_t dst_w, int32_t dst_h)
{
    size_t dst_bytes = (size_t)dst_w * (size_t)dst_h * 4u;

    /* Exact right-angle maps stay inside the rotated content box, so the
     * transparent-outside test in argb_rotate is unnecessary.  Dispatch the
     * shorter argb_blit kernel; padded destinations still use rotate
     * because blit clamps out-of-range samples instead of writing
     * transparent 0. */
    if ((rot % 90) == 0 && dst_w <= bw && dst_h <= bh) {
        /* 180's backward walk streams at DRAM bandwidth even past the
         * cliff (measured), so only the forward and gather walks are
         * tiled. */
        if (rot != 180 && dst_bytes > G2D_BIG_SURFACE)
            return gpu_rotate_tiled(g2d_qpu_argb_blit, g2d_qpu_argb_blit_n,
                                    m, src_phys, argb_src, src_w, src_h,
                                    dst_phys, argb_dst, dst_w, dst_h, 1);
        return gpu_blit_surface(m, src_phys, argb_src, src_w, src_h,
                                dst_phys, argb_dst, dst_w, dst_h,
                                0, 0, dst_w, dst_h, 1, V3D_G2D_MAINT_ALL);
    }
    if (dst_bytes > G2D_BIG_SURFACE)
        return gpu_rotate_tiled(g2d_qpu_argb_rotate, g2d_qpu_argb_rotate_n,
                                m, src_phys, argb_src, src_w, src_h,
                                dst_phys, argb_dst, dst_w, dst_h, 0);
