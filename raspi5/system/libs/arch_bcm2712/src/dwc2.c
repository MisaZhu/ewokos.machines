/*
 * dwc2.c: polled DWC2 OTG host controller driver for the BCM2712 SoC.
 *
 * Port of machines/raspix/system/libs/arch_bcm283x/src/dwc2.c. bcm2712.dtsi
 * gives usb@480000 the very same "brcm,bcm2835-usb" compatible as bcm283x, so
 * the register programming is unchanged: UTMI 8-bit PHY selection
 * (USBTRDTIM=9), per-stage control retries, toggle resync from HCTSIZ.PID and
 * the FS/LS-only latch for boards with a dead HS data path.
 *
 * What does differ on BCM2712:
 *   - the register file is its own 64 KB window at 0x10_00480000, mapped here
 *     through SYS_MEM_MAP instead of being an offset inside the main
 *     peripheral window;
 *   - DMA is identity. The dtsi node hangs off the root, which has no
 *     dma-ranges, so HCDMA takes the plain CPU physical address — no
 *     0xC0000000 VC alias like bcm283x needs;
 *   - the core lives in a firmware power domain and only decodes after a VPU
 *     mailbox power-on, so init probes GSNPSID and gives up cleanly when the
 *     core never answers (a plain Pi 5 does not wire these USB2 pins out).
 */
...
/* DWC2 FIFO size registers use 4-byte words, not bytes.
   Linux dwc2_set_bcm_params() programs exactly these three values for the
   "brcm,bcm2835-usb" compatible, i.e. for this very core; they are clamped
   against the FIFO RAM depth GHWCFG3 reports at init time. */
...
/* BCM2712 uses an internal 8-bit UTMI PHY, same as BCM283x. Linux programs
...
/* GSNPSID of a live Synopsys DWC2 core reads 0x4f54xxxx ("OT" + version);
   an unpowered or absent one answers 0 or all-ones. */
#define DWC_GSNPSID_OTG_MAGIC 0x4F540000u
#define DWC_GSNPSID_MAGIC_MASK 0xFFFF0000u

#define BCM2712_MBOX_TAG_SET_POWER_STATE 0x00028001u
#define BCM2712_MBOX_POWER_STATE_ON (1u << 0)
#define BCM2712_MBOX_POWER_STATE_WAIT (1u << 1)
/* response state bit 1: the firmware does not know this device id */
#define BCM2712_MBOX_POWER_STATE_NODEV (1u << 1)

/*
 * The firmware numbers SET_POWER_STATE devices two different ways and both
 * are still in circulation: the mailbox property-interface wiki lists
 * 3 = "USB HCD" (the id bcm283x has always used), while Linux's
 * raspberrypi-power.c — the driver bcm2712-rpi.dtsi binds &usb to — numbers
 * RPI_POWER_DOMAIN_USB = 6. Both are requested and the core itself decides
 * which one mattered; the wiki id 6 is I2C2 and the Linux id 3 is the video
 * scaler, so neither attempt can hurt anything that is already running.
 */
#define BCM2712_POWER_ID_USB_HCD 3u
#define BCM2712_POWER_ID_USB_DOMAIN 6u
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t val_len;
} bcm2712_mbox_tag_hdr_t;

typedef struct __attribute__((packed)) {
    bcm2712_mbox_tag_hdr_t tag_hdr;
    struct {
        uint32_t device_id;
        uint32_t state;
    } body;
} bcm2712_mbox_tag_set_power_state_t;

typedef struct __attribute__((packed)) {
    bcm2712_mbox_hdr_t hdr;
    bcm2712_mbox_tag_set_power_state_t set_power_state;
    uint32_t end_tag;
} bcm2712_mbox_power_msg_t;
...
/*
 * Maps the main peripheral window (the VPU mailbox lives there) plus the SoC
 * USB2 window of its own. sys_mem_map() returns 0 and installs *nothing*
 * when the request misses check_mem_map_arch()'s whitelist, so both results
 * have to be checked: an unchecked failure would turn the first GSNPSID read
 * into a data abort that kills the daemon, and a dead child leaves ipcserv
 * spinning in ipc_ping() forever.
 */
static int dwc2_map_windows(void) {
    ewokos_addr_t base;
    ...
    if (mmio_map() == 0) {
        klog("dwc2: main mmio map failed\n");
        return -1;
    }
    base = _mmio_base + PI5_USB_DWC2_WIN_OFF;
    if (syscall3(SYS_MEM_MAP, base, PI5_USB_DWC2_PHY,
            PI5_USB_DWC2_WIN_SIZE) != base) {
        klog("dwc2: usb2 window map failed\n");
        return -1;
    }
    _usb_base = base;
    _mapped = true;
...
static bool dwc2_core_present(void) {
    uint32_t id;

    if (!_mapped) {
        return false;
    }
    id = usb_readl(DWC_REG_GSNPSID);
...
/*
 * Identity DMA: usb@480000 sits at the root of bcm2712.dtsi, which carries no
 * dma-ranges, so the HCDMA register takes the plain CPU physical address.
 * bcm283x has to fold in the 0xC0000000 uncached VC alias here; BCM2712 does
 * not, and the sys_dma pool is carved out of low RAM so 32 bits always cover
 * it (HCDMA is a 32-bit register).
 */
...
static int dma_pool_init(void) {
    ...
    phy = dma_phy_addr(0, _dma_pool.virt);
    if (phy == 0 || phy > 0xFFFFFFFFull) {
        klog("dwc2: dma pool outside 32bit window\n");
        return -1;
    }
    _dma_pool.phys = (uint32_t)phy;
...
/* returns 0 when the firmware accepted the request, -1 otherwise */
static int bcm2712_set_power_state(uint32_t device_id) {
    mail_message_t msg;
    bcm2712_mbox_power_msg_t* req;
    ewokos_addr_t phy;
    uint32_t resp_state;

    req = (bcm2712_mbox_power_msg_t*)(uintptr_t)dma_alloc(0, sizeof(*req));
    if (req == NULL) {
        return -1;
    }
    ...
    req->hdr.code = 0; /* request */
    req->set_power_state.tag_hdr.tag = BCM2712_MBOX_TAG_SET_POWER_STATE;
    req->set_power_state.tag_hdr.val_buf_size =
            sizeof(req->set_power_state.body);
    req->set_power_state.tag_hdr.val_len =
            sizeof(req->set_power_state.body);
    req->set_power_state.body.device_id = device_id;
    req->set_power_state.body.state =
            BCM2712_MBOX_POWER_STATE_ON | BCM2712_MBOX_POWER_STATE_WAIT;

    /* the mailbox buffer address is a 28-bit VC bus address: the sys_dma pool
       sits in low RAM, but truncating an out-of-range one would silently hand
       the firmware somebody else's memory */
    phy = dma_phy_addr(0, (ewokos_addr_t)(uintptr_t)req);
    if (phy == 0 || phy > 0xFFFFFFFFull) {
        return -1;
    }
    memset(&msg, 0, sizeof(msg));
    msg.data = (uint32_t)(((uint32_t)phy | MAILBOX_VC_ALIAS_NONCACHED) >> 4);
    msg.channel = PROPERTY_CHANNEL;
    bcm2712_mailbox_call(&msg);

    resp_state = req->set_power_state.body.state;
    if (resp_state & BCM2712_MBOX_POWER_STATE_NODEV) {
        return -1;
    }
    return (resp_state & BCM2712_MBOX_POWER_STATE_ON) ? 0 : -1;
}

/*
 * Power the core on and wait until it answers GSNPSID. The power request goes
 * out before the first register read on purpose: reading a power-gated block
 * is what could fault, and both candidate ids are harmless to switch on.
 */
static int dwc2_power_on_core(void) {
    static const uint32_t ids[] = {
        BCM2712_POWER_ID_USB_HCD,
        BCM2712_POWER_ID_USB_DOMAIN,
    };
...
/*
 * GHWCFG3[31:16] reports the shared FIFO RAM depth in words. 774+256+512 is
 * what Linux programs for this core and what BCM2835 runs stable with, but a
 * deeper-programmed-than-present RAM silently truncates transfers, so scale
 * the three sizes down proportionally if the core reports less.
 */
static void dwc_program_fifos(void) {
    uint32_t rx = DWC_RX_FIFO_SIZE;
    uint32_t nptx = DWC_NP_TX_FIFO_SIZE;
    uint32_t ptx = DWC_P_TX_FIFO_SIZE;
    uint32_t want = rx + nptx + ptx;
    uint32_t have = (usb_readl(DWC_REG_GHWCFG3) >> 16) & 0xFFFFu;

    if (have != 0 && want > have) {
        uint32_t new_rx = (uint32_t)(((uint64_t)rx * have) / want);
        uint32_t new_nptx = (uint32_t)(((uint64_t)nptx * have) / want);
        klog("dwc2: fifo ram %u words < %u, scaling rx=%u nptx=%u\n",
                have, want, new_rx, new_nptx);
        rx = new_rx;
        nptx = new_nptx;
        ptx = (have > rx + nptx) ? (have - rx - nptx) : 0u;
    }

    usb_writel(DWC_REG_GRXFSIZ, rx);
    usb_writel(DWC_REG_GNPTXFSIZ, (nptx << 16) | rx);
    usb_writel(DWC_REG_HPTXFSIZ, (ptx << 16) | (rx + nptx));
}
...
int dwc2_reset_port(void) {
    ...
    /* PrtSpd is HPRT[18:17]: 00=high, 01=full, 10=low. Decoding it straight
       matters — a low_speed bool alone reports "full" for a high-speed link
       that cannot actually move data. */
    speed_bits = (reg & DWC_HPRT_SPEED_MASK) >> DWC_HPRT_SPEED_SHIFT;
    low_speed = speed_bits == 2u;
    /* BCM2712 uses the internal HS UTMI PHY. FS uses 30/60MHz. LS keeps
...
static int dwc_host_init(void) {
    ...
    /* Internal PHY on BCM2712 is UTMI+ 8-bit: clear ULPI_UTMI_SEL and PHYIF
       (Linux dwc2 params_bcm does the same for "brcm,bcm2835-usb"). Selecting
       ULPI here leaves the PHY unclocked (no connect detect). */
    ...
    while ((usb_readl(DWC_REG_GINTSTS) & DWC_GINTSTS_CURMODE_HOST) == 0) {
        if (waited_ms++ >= 100u) {
...
int dwc2_init(void) {
    ...
    if (dwc2_map_windows() != 0) {
        return -1;
    }
    if (dwc2_power_on_core() != 0) {
        /* a plain Pi 5 does not wire the SoC USB2 pins out; the caller
           degrades to "no dwc2 port" and keeps running on the RP1 xHCI */
        klog("dwc2: core not present (gsnpsid=0x%08x)\n",
                usb_readl(DWC_REG_GSNPSID));
        return -1;
    }
    if ((usb_readl(DWC_REG_GSNPSID) & DWC_GSNPSID_MAGIC_MASK) !=
            DWC_GSNPSID_OTG_MAGIC) {
        klog("dwc2: unexpected gsnpsid=0x%08x\n",
                usb_readl(DWC_REG_GSNPSID));
    }
    if (dma_pool_init() != 0) {
        return -1;
    ...
    _ready = true;
    klog("dwc2: host up, %u channels, gsnpsid=0x%08x ghwcfg2=0x%08x\n",
            _num_host_channels, usb_readl(DWC_REG_GSNPSID),
            usb_readl(DWC_REG_GHWCFG2));
    return 0;
}
