/*
 * Polled xHCI driver for the RP1 DWC3 host controllers (Raspberry Pi 5).
 * See xhci.h for the big picture. Everything here follows the xHCI 1.1
 * spec register/TRB layout; no DWC3 global registers are touched because
 * the firmware already left the cores in host mode.
 *
 ...
 *
 * All bus addresses handed to the controller carry the PCIe inbound
 * window offset (+0x10_0000_0000); the sys_dma pool below RAM top is
...
/* CPU physical -> RP1/PCIe bus address (bcm2712.dtsi pcie2 dma-ranges) */
#define RP1_PCIE_DMA_OFFSET 0x1000000000ULL
