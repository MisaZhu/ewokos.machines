 ...
 *
 * Pi 5 USB is nothing like the Pi 4 and earlier: the four type-A ports hang
 * off two Synopsys DWC3 controllers inside RP1 (rp1.dtsi usb@200000 and
 * usb@300000), each exposing a standard xHCI 1.x host register file at the
 * base of its 1MB window. The firmware bootloader already brings the DWC3
 * cores up in host mode (it enumerates USB itself for usb-boot), so a plain
 * xHCI init - HCRST included - is all that is needed; no DWC3 global
 * register setup, no mailbox power-on, no DWC2 channel machinery.
 *
 * The driver is fully polled: no interrupts are enabled, the caller drains
 * the event ring from its main loop. Unlike the old DWC2 code, interrupt-IN
 * endpoints do not need software NAK pacing - the controller schedules the
 * periodic transactions in hardware and only posts a transfer event when a
 * TD actually completes.
 *
 * DMA: RP1 masters sit behind PCIe; the inbound window maps CPU physical
 * 0x0 at PCIe address 0x10_0000_0000 (bcm2712.dtsi pcie2 dma-ranges), so
 * every address handed to the controller carries that offset. All rings,
 * contexts and data buffers come from the sys_dma pool, which the kernel
 * maps uncached into this process (svc.c sys_dma_alloc: PTE_ATTR_NOCACHE),
 ...
 * (CPU physical + 0x10_0000_0000) ready to be handed to the controller.
 */
