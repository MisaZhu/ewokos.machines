...
int uc_cwu50_init(void) {
     ...
     * sequence ran.  The raspi5 arch library does expose a DCS read
     * (bcm2712_rp1_dsi_cmd_read), but this probe's result is thrown away
     * and a read never alters DDIC state, so the write-side ordering above
     * is kept and the read is still dropped.  The panel therefore sees
