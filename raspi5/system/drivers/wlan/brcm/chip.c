...
/* Last transport error seen by a buscore read. Reads keep returning a
 * plain value (callers expect that), but the erom scan checks this so
 * a dead backplane aborts attach in one transfer timeout instead of
 * stalling 2s per descriptor. */
static int brcmf_buscore_read_err;
...
static int brcmf_chip_dmp_erom_scan()
{
    ...
    do{
        brcmf_buscore_read_err = 0;
        eromaddr = brcmf_sdio_buscore_read32(CORE_CC_REG(cc->base, eromptr));
        if (brcmf_buscore_read_err) {
            brcm_log("erom: eromptr read failed %d, backplane dead\n",
                  brcmf_buscore_read_err);
            return brcmf_buscore_read_err;
        }
    }while(eromaddr < SI_ENUM_BASE_DEFAULT);

    while (desc_type != DMP_DESC_EOT) {
        if (brcmf_buscore_read_err) {
            brcm_log("erom: descriptor read failed %d, aborting scan\n",
                  brcmf_buscore_read_err);
            return brcmf_buscore_read_err;
        }
