 ...
 * unmasks SDHCI_INT_CARD_INT in the host, so the controller latches the
 * card's DAT1 assertion into SDHCI_INT_STATUS. sdhci_send_command() clears
 * the whole INT_STATUS word both before and after every transaction, which
 * makes the bit mean exactly "the card rang since my last SDIO command".
 *
 ...
 * the func0 probe whenever this says quiet. That fallback is also what keeps
