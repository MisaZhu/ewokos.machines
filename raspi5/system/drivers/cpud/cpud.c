...
static int mailbox_property_xfer(void* tags, uint32_t tags_size) {
	uint32_t buf_size = sizeof(prop_msg_hdr_t) + tags_size + sizeof(uint32_t);
	...
	for(size_t i = 0; i < sizeof(aliases) / sizeof(aliases[0]); i++) {
		...
		*((uint32_t*)(buf + sizeof(prop_msg_hdr_t) + tags_size)) = PROP_TAG_END;
		...
		if(bcm2712_mailbox_call_timeout(&msg, 0) == 0 &&
				hdr->code == PROP_CODE_RESPONSE_SUCCESS) {
...
static char* cpud_snapshot_json(const cpu_snapshot_t* snap) {
	...
	str_addf(s, "\"mailbox_snapshot_cached\":false,");
	str_addf(s, "\"mailbox_ready\":%s,", bool_str(snap->mailbox_ready));
	str_addf(s, "\"probe_ok\":%s,", bool_str(snap->probe_ok));
	str_addf(s, "\"probe_generation\":%u,", snap->probe_generation);
	str_addf(s, "\"success_count\":%u,", snap->success_count);
	str_addf(s, "\"error_count\":%u,", snap->error_count);
	str_add(s, "\"warning\":\"Pi5 mailbox data is queried when dev.cmd /dev/cpu info runs.\",");
