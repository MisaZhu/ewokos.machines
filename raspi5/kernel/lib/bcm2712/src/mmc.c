...
int mmc_init(void){
	...
	printf("[pi5] sd: querying card (op cond) ...\n");
	if(mmc_get_op_cond(&_mmc) != 0) {
		printf("[pi5] sd: card did not respond!\n");
		return -1;
	}
	...
	_mmc.has_init = true;
	printf("[pi5] sd: card ready, capacity=%d MB, high_capacity=%d\n",
			(uint32_t)((uint64_t)_mmc.capacity * _mmc.read_bl_len / (1024*1024)),
			_mmc.high_capacity?1:0);
	return 0;
}
