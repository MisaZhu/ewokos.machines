...
struct bus_ops* bcm2712_sdhci_init(void)
{
	 ...
	 */
	printf("[pi5] sd: EMMC host @ 0x%08x\n", (uint32_t)PI5_EMMC_BASE);
	...
	printf("[pi5] sd: host version=0x%x caps=0x%x caps1=0x%x max_clk=%d\n",
		readw((uint32_t*)(_host.ioaddr + SDHCI_HOST_VERSION)),
		readl(_host.ioaddr + SDHCI_CAPABILITIES),
		readl(_host.ioaddr + SDHCI_CAPABILITIES_1),
		_host.max_clk);
