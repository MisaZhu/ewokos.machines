...
static const char* _conf_file = "/etc/display.i2c.json";
...
int main(int argc, char** argv) {
    ...
    if(ssd1306_init() != 0) {
        fprintf(stderr, "i2cdisplayd: failed to init SSD1306 on i2c bus %d addr 0x%02x\n",
                _i2c_bus, _i2c_addr);
        return -1;
    }
    ...
    return fbdisplayd_run(&display, mnt_point, OLED_WIDTH, OLED_HEIGHT, _conf_file, _display_index);
}
