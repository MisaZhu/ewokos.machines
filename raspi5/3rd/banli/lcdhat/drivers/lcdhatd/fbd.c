...
static int tp_read(uint8_t* buf, uint32_t size) {
	...
	if(size >= 6) {
		...
		/*
		 * Keep LCD CS released after touch access. On raspi5 CE0 is a
		 * software-driven GPIO CS, so pulling it low here would leave the
		 * panel selected outside of an active LCD transaction.
		 */
		bsp_gpio_write(8, 1);
		//klog("tp_read: %d %d %d\n", d[0], d[1], d[2]);
	}
