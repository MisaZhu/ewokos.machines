...
static inline void lcd_reset( void ) {
	...
	/*
	 * Match the vendor reset sequence more closely: keep reset released first,
	 * pulse low briefly, then allow the controller enough time to boot before
	 * the first command.
	 */
...
void ili9486_init(uint16_t w, uint16_t h, uint16_t rot, uint16_t inversion,
	...
	ili9486_set_config(pin_dc, pin_cs, pin_rst, pin_bl, cdiv, SPI_SELECT_LINE);
	bsp_gpio_init();
