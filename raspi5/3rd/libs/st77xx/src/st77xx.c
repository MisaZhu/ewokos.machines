...
static int SPI_SELECT_LINE = SPI_SELECT_0;
...
static inline void lcd_start(void) {
	/*
	 * On raspi5 CE0/CE1 are driven by the SPI backend as software CS.
	 * Avoid double-driving the same line through GPIO when a SPI select
	 * line is active; keep manual GPIO CS only for pure bitbanged/manual use.
	 */
	if(LCD_CS >= 0 && SPI_SELECT_LINE < 0) {
...
void st77xx_set_config(int pin_dc, int pin_cs, int pin_rst, int pin_bl,
...
void st77xx_init(uint16_t w, uint16_t h, uint16_t rot, uint16_t inversion,
	...
	st77xx_set_config(pin_dc, pin_cs, pin_rst, pin_bl, cdiv, SPI_SELECT_LINE);
	bsp_gpio_init();
