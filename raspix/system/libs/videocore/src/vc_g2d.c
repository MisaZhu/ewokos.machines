/*
 * vc_g2d.c - VideoCore (V3D) back end of the raspix (Raspberry Pi 3 /
 * BCM2837, Pi 4 / BCM2711) g2d layer: V3D-accelerated offline ARGB8888
 * drawing, ported from the raspi5 bsp_g2d.c (proven on real Pi 5 hardware).
 *
 ...
 * kernels, the eligibility gate on caller-supplied physical addresses, and
 * every QPU dispatch.  v3d_g2d.c holds the hardware bring-up, cache and
 * submission mechanics; g2d_qpu_kernels.h / g2d_qpu_kernels_v42.h hold the
 ...
 *
 *   gpu_fill             any clipped rect        -> argb_fill kernel
 *   gpu_blit_surface     any clipped dst rect    -> argb_blit kernel
 *   gpu_rotate_surface   any angle, exact bbox   -> argb_rotate kernel
 *
 * gpu_alpha_surface uses a dedicated source-over pipeline.  V3D >= 4
...
/* The three-state GPU result, the rotation codes and g2d_map_t (the Q15
...
/* Constant-color fill of a clipped dst rect: VC4 (V3D 2.1) tries the
 * self-looping group-fill kernel first and falls back to the exact span
 * batches for everything it declines; V3D >= 4 runs the CSD fill kernel. */
                      ...
                      int32_t dst_w, int32_t dst_h, int rot)
{
    ...
    /* V3D >= 4 kernel only: VC4 has no right-angle fast path, and every
     * other angle goes through the generic affine map instead. */
