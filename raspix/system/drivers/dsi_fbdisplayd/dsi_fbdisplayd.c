...
static int32_t init(uint32_t w, uint32_t h, uint32_t dep) {
	...
	/*
	 * Pick the DSI port the panel actually hangs off: DSI0 on the
	 * Pi3 family (this Waveshare 4" board), DSI1 on Pi4/CM4.  A
	 * powered-off port silently drops register writes, so probe
	 * both with a write-readback test in the preferred order and
	 * use the first live one.
	 */
	{
		int first = bcm283x_dsi1_is_gen5() ? 1 : 0;
