...
typedef struct {
	...
	bool debug_logged_gpio;
	bool debug_logged_sample_map;
	bool need_rebuffer;
	bool configured;
	bool prepared;
	bool started;
...
static void audio_detect_hw_config(void) {
	...
	slog("soundpwm: machine=%s mmio=%x pwm=%x fifo=%x permap=%x clk=%u src=%u div=%u.%u pi4=%d\n",
	...
	if (_snd.pcm_cfg.bit_depth == 16) {
		...
		if (!_snd.debug_logged_sample_map) {
			slog("soundpwm: sample_probe bits=16 ch=%d l=%d r=%d pwm_r=%u pwm_l=%u range=%u scale=%u\n",
					...
					_snd.pwm_range,
					_snd.pwm_scale);
			_snd.debug_logged_sample_map = true;
		}
		/* #endregion debug-point soundpwm-sample-probe */
		return;
	}
...
static int audio_start_dma_transfer(uint32_t slot, uint32_t samples, bool is_rebuffer_start) {
	...
	*(dma + DMA_CS) = DMA_ACTIVE | DMA_PRIORITY_DEFAULT | DMA_PANIC_PRIORITY_DEFAULT;
	...
	slog("soundpwm: dma start slot=%u samples=%u cb=%x pwm_fifo=%x\n",
...
int main(int argc, char** argv) {
	...
	strcpy(dev.name, "bcm283x-pwm-snd");
	dev.open = sound_open;
