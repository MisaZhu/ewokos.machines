...
void bcm283x_gpio_config(int32_t no, int32_t gpio_sel) {
    ewokos_addr_t raddr = (ewokos_addr_t)GPIO_FSEL0 + ((no/10)<<2);
...
void bcm283x_gpio_pull(int32_t no, int32_t pull_dir) {
    ...
    *GPIO_PUD = 0;
    put32((ewokos_addr_t)GPIO_PUD + (index<<2), 0); /* disable ppud clock */
}

inline void bcm283x_gpio_set(int32_t no) {
    put32((ewokos_addr_t)GPIO_SET0 + ((no/32)<<2), 1<<(no%32));
}

inline void bcm283x_gpio_clr(int32_t no) {
    put32((ewokos_addr_t)GPIO_CLR0 + ((no/32)<<2), 1<<(no%32));
}
...
inline uint32_t bcm283x_gpio_read(int32_t no) {
    if((get32((ewokos_addr_t)GPIO_LEV0 + ((no/32)<<2)) & (1<<(no%32))) != 0)
        return 1;
    return 0;
}
