 ...
 *
 * The LCD and the XPT2046 share SPI0: the LCD CS is parked deselected for
 * the duration of a touch poll so stray touch bytes never reach the panel's
 * GRAM. Any sample clobbered by a preempting flush IPC is rejected by the
 * clamp filter.
 */
