 ...
 * 0xC00000 that fbdisplayd.workaround used) will silently fail SYS_MEM_MAP
 * and take the whole fbdisplayd process down with init()==-1.
 *
 ...
 * right on top of this framebuffer, so firmware and fbdisplayd would trample
...
/*
 * `devcmd /dev/disp0` knobs for the things that belong to the panel rather
 ...
 * log, flush the log to /fbdisplayd.log on the SD card (synchronous ext2
...
int main(int argc, char** argv) {
    ...
    _fbdisplayd_cfg.splash = NULL;   /* default logo splash from libfbdisplayd */
    ...
    contrast_build_lut();
    contrast_sync_fast_paths();
    /* Panel-only knobs (backlight, contrast) via `devcmd /dev/disp0`. */
    fbdisplayd_set_dev_cmd(fb6d_dev_cmd);
